...
if (document.body.classList.contains('home')) {
    ...
    // let fillRadius = calcFillRadius(circleX, circleY);
    ...
    ScrollTrigger.matchMedia({
        ...
        "(max-width: 1199px)": function () {
            ...
            // Timeline Animation (include onUpdate function here)
            circleTl.to(homeCircle, {
                radius: () => calcFillRadius(vw, vh) + 50,
                onUpdate: () => { drawCircle(shapeCanvasContext, homeCircle) }
            });
            circleTl.to('.hero-arrow-container',
                {
...
if (document.body.classList.contains('project-body')) {
    ...
    window.addEventListener('load', () => {
        ...
        circleTl.to(circle, {
            ...
            onUpdate: drawCircle(shapeCanvasContext)
        });
...
function calcFillRadius(viewWidth, viewHeight) {
    let circleX = calcCircleX(viewWidth);
    let circleY = calcCircleY(viewWidth, viewHeight);
...
function updateCircle(context, circle) {
    ...
    let circleX = calcCircleX(vw);
    let circleY = calcCircleY(vw, vh);
    let fillRadius = calcFillRadius(vw, vh);
